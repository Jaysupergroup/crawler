  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <section className="card explorer">
        <div className="explorer-head"><div><p className="eyebrow">{explorerView === 'pages' ? 'Audited pages' : explorerView === 'links' ? 'Discovered links & anchors' : explorerView === 'resources' ? 'Resources & assets' : explorerView === 'issues' ? 'SEO issues' : explorerView === 'comparison' ? 'Crawl comparison' : 'Saved crawl history'}</p><h2>{explorerView === 'pages' ? `${crawler.pages.length.toLocaleString()} page${crawler.pages.length === 1 ? '' : 's'} collected` : explorerView === 'links' ? `${crawler.links.length.toLocaleString()} link${crawler.links.length === 1 ? '' : 's'} collected` : explorerView === 'resources' ? 'Embedded resource inventory' : explorerView === 'issues' ? `${issueCount.toLocaleString()} issue${issueCount === 1 ? '' : 's'} identified` : explorerView === 'comparison' ? `${crawlComparison?.rows.length.toLocaleString() || 0} changes identified` : 'Crawls retained in MySQL'}</h2></div><div className="export-wrap"><button className="secondary" onClick={() => setExportOpen(open => !open)}>Export ▾</button>{exportOpen && <div className="export-menu">{[
        ...
        {explorerView === 'pages' ? <PagesExplorer pages={crawler.pages} sharedSearch={search} onInspectPage={(page, section) => { setSelectedPageTab(section); setSelectedPage(page); }} /> : explorerView === 'links' ? <LinksExplorer links={crawler.links} sharedSearch={search} /> : explorerView === 'resources' ? <ResourcesExplorer pages={crawler.pages} sharedSearch={search} /> : explorerView === 'issues' ? <IssuesExplorer issues={seoIssues} sharedSearch={search} onInspectPage={url => { const target = crawler.pages.find(page => page.url === url); if (target) { setSelectedPageTab('overview'); setSelectedPage(target); } }} /> : explorerView === 'comparison' && crawlComparison ? <ComparisonExplorer comparison={crawlComparison} onBack={() => setExplorerView('history')} /> : <HistoryExplorer onRestore={async (record: CrawlHistoryRecord) => { setRestoringAudit({ pageCount: record.stats?.pagesCrawled || 0, expectedPages: null, stage: 'restoring' }); try { const restored = await crawler.restoreHistory(record.id); setConfig(current => ({ ...current, ...(restored.crawl.config || {}), seedUrl: restored.crawl.seedUrl })); setExplorerView('pages'); setRestoringAudit({ pageCount: restored.restoredPages, expectedPages: restored.restoredPages, stage: 'rendering' }); } catch (error) { setRestoringAudit(null); throw error; } }} onCompare={async (previous, current) => { setComparingHistory(true); try { const result = await crawlerClient.compareHistory(previous.id, current.id); setCrawlComparison(result); setExplorerView('comparison'); } finally { setComparingHistory(false); } }} />}
      </section>
    </main>
