  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <section className="card explorer">
        ...
        {explorerView === 'pages' ? crawler.historyAudit ? <SavedAuditPagesExplorer crawlId={crawler.historyAudit.crawlId} sharedSearch={search} onInspectPage={async (page, section) => { const detail = await crawlerClient.historyPage(crawler.historyAudit!.crawlId, page.url); setSelectedPageTab(section); setSelectedPage(detail.page); }} /> : <PagesExplorer pages={crawler.pages} sharedSearch={search} onInspectPage={(page, section) => { setSelectedPageTab(section); setSelectedPage(page); }} /> : explorerView === 'links' ? crawler.historyAudit ? <SavedAuditLinksExplorer crawlId={crawler.historyAudit.crawlId} sharedSearch={search} /> : <LinksExplorer links={crawler.links} sharedSearch={search} /> : explorerView === 'resources' ? <ResourcesExplorer pages={crawler.pages} sharedSearch={search} /> : explorerView === 'issues' ? <IssuesExplorer issues={seoIssues} sharedSearch={search} onInspectPage={url => { const target = crawler.pages.find(page => page.url === url); if (target) { setSelectedPageTab('overview'); setSelectedPage(target); } }} /> : explorerView === 'comparison' && crawlComparison ? <ComparisonExplorer comparison={crawlComparison} onBack={() => setExplorerView('history')} /> : <HistoryExplorer onRestore={async (record: CrawlHistoryRecord) => { setRestoringAudit({ pageCount: record.stats?.pagesCrawled || 0, expectedPages: null, stage: 'restoring' }); try { const restored = await crawler.restoreHistory(record.id); setConfig(current => ({ ...current, ...(restored.crawl.config || {}), seedUrl: restored.crawl.seedUrl })); setExplorerView('pages'); setRestoringAudit({ pageCount: restored.restoredPages, expectedPages: restored.loadedPages, stage: 'rendering' }); } catch (error) { setRestoringAudit(null); throw error; } }} onCompare={async (previous, current) => { setComparingHistory(true); try { const result = await crawlerClient.compareHistory(previous.id, current.id); setCrawlComparison(result); setExplorerView('comparison'); } finally { setComparingHistory(false); } }} />}
      </section>
    </main>
