...
import type { CrawlConfig, CrawlHistoryRecord, CrawlPage, CrawlScope, HtmlComparisonCapture } from './types/crawl';
import './styles.css';
...
export default function App() {
  ...
  const [explorerView, setExplorerView] = useState<'pages' | 'links' | 'resources' | 'issues' | 'history'>('pages');
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <section className="card explorer">
        ...
        {explorerView !== 'history' && <div className="toolbar"><input value={search} onChange={event => setSearch(event.target.value)} placeholder={explorerView === 'pages' ? 'Search URLs, titles, descriptions or status codes…' : explorerView === 'links' ? 'Search anchor text, URLs or status codes…' : explorerView === 'resources' ? 'Search resource URLs, types, source pages or status…' : 'Search issue names, URLs, details or severity…'} /></div>}
        {explorerView === 'pages' ? <PagesExplorer pages={crawler.pages} sharedSearch={search} onInspectPage={(page, section) => { setSelectedPageTab(section); setSelectedPage(page); }} /> : explorerView === 'links' ? <LinksExplorer links={crawler.links} sharedSearch={search} /> : explorerView === 'resources' ? <ResourcesExplorer pages={crawler.pages} sharedSearch={search} /> : explorerView === 'issues' ? <IssuesExplorer issues={seoIssues} sharedSearch={search} onInspectPage={url => { const target = crawler.pages.find(page => page.url === url); if (target) { setSelectedPageTab('overview'); setSelectedPage(target); } }} /> : <HistoryExplorer onRestore={async (record: CrawlHistoryRecord) => { setRestoringAudit({ pageCount: record.stats?.pagesCrawled || 0, expectedPages: null, stage: 'restoring' }); try { const restored = await crawler.restoreHistory(record.id); setConfig(current => ({ ...current, ...(restored.crawl.config || {}), seedUrl: restored.crawl.seedUrl })); setExplorerView('pages'); setRestoringAudit({ pageCount: restored.restoredPages, expectedPages: restored.restoredPages, stage: 'rendering' }); } catch (error) { setRestoringAudit(null); throw error; } }} />}
      </section>
    </main>
